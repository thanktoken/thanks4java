...
public class ThankTokenBeanTest extends TestCase {
  ...
  public void testGetValueTenYears() {
    ...
    ThankHeaderBean header = new ThankHeaderBean();
    header.setAmount(amount);
    header.setTimestamp(OffsetDateTime.now().minusDays(3652).toInstant());
    ThankTokenBean token = new ThankTokenBean(header);
  ...
  public void testGetValueHundredYears() {
    ...
    ThankHeaderBean header = new ThankHeaderBean();
    header.setAmount(amount);
    header.setTimestamp(OffsetDateTime.now().minusDays(36524).toInstant());
    ThankTokenBean token = new ThankTokenBean(header);
  ...
  public void testGetValueThousandYears() {
    ...
    ThankHeaderBean header = new ThankHeaderBean();
    header.setAmount(amount);
    header.setTimestamp(OffsetDateTime.now().minusDays(365242).toInstant());
    ThankTokenBean token = new ThankTokenBean(header);
